...
@section('content')
<div class="max-w-4xl">
    <form action="{{ route('admin.pages.store') }}" method="POST" class="glass-card p-10 space-y-8">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2">
                <label class="text-[10px] uppercase font-black tracking-widest text-gray-500">Page Name</label>
                <input type="text" name="name" placeholder="e.g. About Us" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-[#d4a574] transition-all text-white font-bold" required>
            </div>
            
            <div class="space-y-2">
                <label class="text-[10px] uppercase font-black tracking-widest text-gray-500">URL Slug</label>
                <input type="text" name="slug" placeholder="e.g. about/founder" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-[#d4a574] transition-all text-white font-mono" required>
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <textarea name="html_content" rows="15" class="w-full bg-[#0a0506] border border-white/5 rounded-2xl px-6 py-4 outline-none focus:border-[#d4a574] transition-all text-gray-300 font-mono text-sm leading-relaxed" placeholder="Paste your HTML content here..."></textarea>
        ...
        <div class="flex gap-4 pt-6">
            <button type="submit" class="px-10 py-4 bg-[#d4a574] text-black font-black rounded-2xl hover:scale-105 transition-all shadow-lg flex items-center gap-2">
                <i data-lucide="save" class="w-5 h-5"></i> SAVE PAGE
            </button>
