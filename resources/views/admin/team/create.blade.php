...
<div class="glass-card p-8 max-w-5xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h4 class="text-2xl font-black uppercase tracking-tight mb-1">New Authority Profile</h4>
            <p class="text-xs text-gray-500 uppercase tracking-widest">Create an SEO-optimized team identity</p>
        </div>
    ...
    <form action="{{ route('admin.team.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name_input" value="{{ old('name') }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">
            </div>
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug_input" value="{{ old('slug') }}" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">
            </div>

            <!-- Role -->
            <div>
                <label class="block text-xs font-bold text-[#d4a574] uppercase tracking-widest mb-2 ml-1">Role / Job Title *</label>
                <input type="text" name="role" value="{{ old('role') }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">
            </div>
            
            <!-- Sort Order -->
            <div>
                <label class="block text-xs font-bold text-[#d4a574] uppercase tracking-widest mb-2 ml-1">Order Index *</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">
            </div>
        </div>
        ...
        <div class="mt-6">
            ...
            <div id="editor-container" class="bg-[#0a0506] border border-white/10 rounded-xl overflow-hidden">
                <div id="editor" style="height: 300px;">{!! old('bio') !!}</div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div>
                ...
                <textarea name="expertise" rows="3" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">{{ old('expertise') }}</textarea>
            </div>

            <!-- Achievements -->
            <div>
                <label class="block text-xs font-bold text-[#d4a574] uppercase tracking-widest mb-2 ml-1">Key Achievements</label>
                <textarea name="achievements" rows="3" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#d4a574] transition-colors">{{ old('achievements') }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t border-white/5 pt-8 mt-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div>
                    ...
                    <input type="text" name="linkedin_url" value="{{ old('linkedin_url') }}" placeholder="URL" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574] text-sm">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 ml-1">Twitter/X</label>
                    <input type="text" name="twitter_url" value="{{ old('twitter_url') }}" placeholder="URL" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574] text-sm">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 ml-1">Instagram</label>
                    <input type="text" name="instagram_url" value="{{ old('instagram_url') }}" placeholder="URL" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574] text-sm">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 ml-1">GitHub</label>
                    <input type="text" name="github_url" value="{{ old('github_url') }}" placeholder="URL" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574] text-sm">
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-white/5 pt-8 mt-8 bg-white/5 p-6 rounded-2xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574]">
                </div>
                <div>
                    ...
                    <input type="text" name="meta_keywords" value="{{ old('meta_keywords') }}" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574]">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="meta_description" rows="2" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d4a574]">{{ old('meta_description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="schema_markup" rows="4" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-gray-300 font-mono text-xs focus:outline-none focus:border-[#d4a574]">{{ old('schema_markup') }}</textarea>
        ...
        <div class="border-t border-white/5 pt-8 mt-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Image -->
                <div>
                    <label class="block text-xs font-bold text-[#d4a574] uppercase tracking-widest mb-2 ml-1">High-Res Profile Photo</label>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="gradient" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 appearance-none text-white focus:outline-none focus:border-[#d4a574] transition-colors cursor-pointer" style="background-color: #1a0f11;">
                            <option value="from-[#3B0000] to-[#4a1520]">Red Velvet (Founder Style)</option>
                            <option value="from-[#4a1520] to-[#d4a574]">Eclipse Gold (CEO Style)</option>
                            <option value="from-[#d4a574] to-[#e8b44a]">Midas Touch (Exec Style)</option>
                            <option value="from-[#0a0506] to-[#d4a574]">Brutalist Black (Tech Style)</option>
                        </select>
        ...
        <div class="border-t border-white/5 pt-8 mt-8 flex items-center justify-between">
            ...
            <button type="submit" class="bg-gradient-to-r from-[#d4a574] to-[#e8b44a] text-[#2b0e14] px-12 py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform flex items-center gap-3 shadow-2xl shadow-[#d4a574]/30">
                <i data-lucide="shield-check" class="w-5 h-5"></i> Launch Identity
            </button>
        </div>
    </form>
</div>
